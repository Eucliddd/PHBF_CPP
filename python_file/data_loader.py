...
def load_higgs():
    ...
    pd.DataFrame(x_train).to_csv("./data/HIGGS/HIGGS_train.csv", index=False)
    pd.DataFrame(x_test).to_csv("./data/HIGGS/HIGGS_test.csv", index=False)
...
def load_malicious_urls(posNum=16000, negNum=2700):
    ds = pd.read_csv("./data/malicious_urls/All.csv")
    ds.dropna(inplace=True)
    x = ds.drop(["URL_Type_obf_Type"], axis=1)
    x = x.to_numpy()
    y = ds["URL_Type_obf_Type"].to_numpy()
    ...
    pd.DataFrame(x_train).to_csv("./data/malicious_urls/malicious_urls_train.csv", index=False)
    pd.DataFrame(x_test).to_csv("./data/malicious_urls/malicious_urls_test.csv", index=False)

    logger.info(f"Malicious URLs train size = {x_train.shape[0]}")
